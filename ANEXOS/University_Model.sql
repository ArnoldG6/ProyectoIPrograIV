...
ALTER TABLE `university`.`groups` 
ADD COLUMN `teachers_tea_id` VARCHAR(9) NOT NULL AFTER `num_stu`,
DROP PRIMARY KEY,
ADD PRIMARY KEY (`gro_id`, `teachers_tea_id`),
ADD INDEX `fk_groups_teachers1_idx` (`teachers_tea_id` ASC) VISIBLE;
;
...
ADD CONSTRAINT `fk_groups_teachers1`
  FOREIGN KEY (`teachers_tea_id`)
  REFERENCES `university`.`teachers` (`tea_id`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;
